...
class Datasheet():
    ...
    def revenue_sum_formulas(self, df):
        ...
        df = self.sum_formula(df, 'Other Operating Sub-Total', ['other_ad', 'other_gas', 'other_prop', 'other_int', 'other_rev'])
        df = self.sum_formula(df, 'Total (Excludes Capital Revenues)', ['sales_tax', 'dgf_prop_tax','ut_tax', 'mvet', 'Farebox Revenues',
            'fta_5307_op' ,'fta_5307_prv', 'fta_5311_op', 'fta_5316_op', 'fta_other_op', 'def_grnt', 'st_op_rmg',  'st_op_regmg',
            'st_op_sng', 'st_op_ste', 'st_op_other', 'st_gas_tax', 'st_oth_tax', 'Other Operating Sub-Total'])
    ...
    def pretty_formatting(self, df, year1, year2, year3):
        ...
        bad_indices = ['rvh', 'tvh', 'rvm', 'tvm', 'psgr', 'vt', 'dfc', 'bdfc', 'fte']
...
def main(year1, year2, year3):
    ...
    for ferry in ferry_list:
        ...
        df = df.replace('$0.0', '$0')
        df.to_excel(r'I:\Public_Transportation\Data_Team\PT_Summary\PythonFiles\testfolder\ferry\{}.xlsx'.format(ferry), index = False)
