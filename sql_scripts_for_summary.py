...
Sum(oex_do_light+oex_pt_light+oex_do_SR+oex_pt_SR+oex_pt_com) As All_Rail_Modes FROM ptsummary_transit.transit_data where Yr = {} Group By Agnc'''

sw_fin_exp_revenues = '''Select Agnc, SUM(fta_5307_cg+fta_5309_cg+fta_5310_cg+fta_5311_cg+fta_5316_cg+Fstp_grnt_cg+
fed_other_grnt_cg+st_cg_rmg+st_cg_regmg+st_cg_sng+st_cg_ste+st_ct_van+st_cg_other) AS Capital_Expenses FROM ptsummary_transit.revenues where Yr = {} Group By Agnc'''

sw_fin_exp_expenses = '''Select Agnc, SUM(debt_ser_fund) As Debt_Service,  Otra_exp_num As Other, deprec AS Depreciation, local_cap From ptsummary_transit.expenses where Yr = {} Group By Agnc'''
...
FROM ptsummary_transit.revenues where Yr = {} group by Agnc;'''
...
+rev_pt_ferry +rev_pt_com + rev_pt_light + rev_pt_route + rev_pt_demand+rev_pt_CB +rev_pt_DT +rev_pt_RB) AS Fare_Revenues FROM ptsummary_transit.transit_data where Yr = {} group by Agnc;'''
...
(sum(rev_do_fixed + rev_pt_fixed)/sum(oex_do_fixed + oex_pt_fixed))*100 As Farebox_Recovery_Ratio from ptsummary_transit.transit_data as t1 inner join ptsummary_transit.agencytype as t2 on t1.Agnc = t2.Agency where t2.agencytype in ('urban', 'rural', 'small urban') and t1.Yr = {} Group by Agnc;'''
...
(sum(rev_do_demand + rev_pt_demand + rev_pt_DT)/sum(oex_do_demand + oex_pt_demand + oex_pt_DT))*100 As Farebox_Recovery_Ratio from ptsummary_transit.transit_data as t1 inner join ptsummary_transit.agencytype as t2 on t1.Agnc = t2.Agency where t2.agencytype in ('urban', 'rural', 'small urban') and t1.Yr = {} Group by Agnc;
'''
...
(rev_do_van/oex_do_van)*100 As Farebox_Recovery_Ratio from ptsummary_transit.transit_data as t1 inner join ptsummary_transit.agencytype as t2 on t1.Agnc = t2.Agency where t2.agencytype in ('urban', 'rural', 'small urban') and t1.Yr = {} Group by Agnc;
'''
...
(rev_pt_com/oex_pt_com)*100 As Farebox_Recovery_Ratio from ptsummary_transit.transit_data as t1 inner join ptsummary_transit.agencytype as t2 on t1.Agnc = t2.Agency where t2.agencytype in ('urban', 'rural', 'small urban') and t1.Yr = {} Group by Agnc;
'''
...
+oex_do_route+oex_pt_route+oex_do_demand+oex_pt_demand+oex_pt_DT+oex_do_van) as Operating_Investments, SUM(rev_do_fixed + rev_pt_fixed + rev_do_RB + rev_pt_RB+ rev_do_CB+ rev_pt_CB+ rev_do_TB+ rev_pt_com+ rev_do_light+ rev_pt_light + rev_do_SR+ rev_pt_SR+ rev_do_route+rev_pt_route+rev_do_demand+rev_pt_demand+rev_pt_DT+rev_do_van) as Farebox_Revenues FROM ptsummary_transit.transit_data as t1 inner join ptsummary_transit.agencytype as t2 on t1.Agnc = t2.Agency where t2.agencytype in ('urban', 'rural', 'small urban') and t1.Yr in {} group by Yr;'''

sw_invest_exp = '''Select Yr, local_cap as Local_Capital_Investment, SUM(Otra_exp_num + interest + principal) as Other_Investment from ptsummary_transit.expenses as t1 inner join ptsummary_transit.agencytype as t2 on t1.Agnc = t2.Agency where t2.agencytype in ('urban', 'rural', 'small urban') and t1.Yr in {} Group by Yr;'''